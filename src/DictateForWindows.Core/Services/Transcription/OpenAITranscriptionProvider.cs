...
public class OpenAITranscriptionProvider : ITranscriptionProvider
{
    ...
    public IReadOnlyList<string> AvailableModels => TranscriptionModels.OpenAiModels;

    public OpenAITranscriptionProvider(string apiKey, HttpClient? httpClient = null,
        string? baseUrl = null, ILogger<OpenAITranscriptionProvider>? logger = null)
    {
        _apiKey = apiKey ?? throw new ArgumentNullException(nameof(apiKey));
        _httpClient = httpClient ?? new HttpClient();
        _baseUrl = baseUrl ?? ApiEndpoints.OpenAiBaseUrl;
        _logger = logger;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var url = $"{_baseUrl.TrimEnd('/')}{ApiEndpoints.OpenAiTranscription}";
    ...
    private async Task<HttpResponseMessage> SendWithRetryAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        for (int attempt = 0; attempt < ApiEndpoints.MaxRetries; attempt++)
        {
            try
            {
                // Clone the request for retry (request can only be sent once)
    ...
    private static bool IsNonRetryableError(System.Net.HttpStatusCode statusCode)
    {
        ...
        {
            System.Net.HttpStatusCode.Unauthorized => true,  // Invalid API key
            System.Net.HttpStatusCode.Forbidden => true,     // Permission denied
            System.Net.HttpStatusCode.BadRequest => true,    // Invalid request
            System.Net.HttpStatusCode.PaymentRequired => true, // Quota exceeded
            _ => false
        };
    }
    ...
    private static TranscriptionResult HandleErrorResponse(System.Net.HttpStatusCode statusCode, string responseJson)
    {
        ...
        if (lowerMessage.Contains("quota") || statusCode == System.Net.HttpStatusCode.PaymentRequired)
        {
